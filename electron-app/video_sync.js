class VideoSync {
  ...
  selectVideos(selectors, prim_sel) {
    ...
    this.videoList.forEach((video) => {
      ...
      video.onerror = (event) => {
        // * Failed to load
        console.log("error", event.target.error.code, event.target.error.message);
      };
  ...
  waitAllReady() {
    ...
    if (this.state === VideoSync.STATE_SYNCING) {
      ...
      if (ready && this.next_state === VideoSync.STATE_PLAYING) {
        console.log("PLAY");
        this.videoList.forEach(video => { if (!video.error) video.play() });
  ...
  pause(timestamp) {
    ...
    console.log("PAUSED")
    this.state = VideoSync.STATE_PAUSED;
    this.videoList.forEach(video => { if (!video.error) video.pause(); });
    prim.currentTime = timestamp;
  }
  ...
  seekTime(timestamp, noprim) {
    ...
    this.videoList.forEach((video) => {
      // * ignore primary
      if (video.error || (noprim && video == prim)) { return; }
